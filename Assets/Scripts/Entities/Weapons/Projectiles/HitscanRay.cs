...
public class HitscanRay
{
    ...
    {
        var enemiesHit = Physics.RaycastAll(origin, dir.normalized, 100, ~LayerMask.GetMask("SeeThrough", "Pickable", "Ignore Raycast")).Where(x => x.collider.gameObject.LayerMatchesWith("Enemy")).Take(maxEnemies).ToArray();
        float objDist;
